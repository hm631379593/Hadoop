...
def DealWithData(facilityname,dta):
    ...
    pred = results.get_prediction(start=nowdata,end=lengthdata+250,dynamic=False)
    ...
    print(data)
    ax = dta[-200:].plot(label='observed')
    data.plot(ax=ax, label='One-step ahead Forecast', alpha=.5)
    plt.show()
    print(list1,list2)
    return list1,list2
